...
You may be interested in [part one][mythic-forge-1]{:target='_blank'} and [part two][mythic-forge-2]{:target='_blank'} of the project history.
If you're hurry, Mythic forge is an online multi-player game engine, where the game designers use online tools to create and make their game evolve, while players are playing.
...
And since mid-2011, my daily job was about [Node.js][node]{:target='_blank'}, JavaScript MVC frameworks and [MongoDB][mongo]{:target='_blank'}.
...
This was a really happy experience, especially because SmartData has to handle and adapt to unstructured data.
...
Dynamic languages are fancy nowadays, and they might have better productivity.
With Node.js, I really experienced that.

In addition to the [previous features][features]{:target='_blank'}, Hyperions add notable novelties:

- In-game animations with sprites
- Simplified rule writing with changes detections on object and auto-save
- Meaningfull rules with custom object properties without the use of getter/setter
...
And to ease game designers work when authoring their UI, Atlas was transformed to a framework-agnostic JavaScript library.
It's intended to be embed in the Game UI, and takes care of state sharing above the WebSocket channel.
It comes without dependencies (in exception of Socket.io client TODO), to let the designer free of using any JavaScript stack.

TODO Atlas internals

# A space crusade with Deep-Hulk

With "Tales of Eethia 2" failure, I did not wanted to put to much efforts on writing a game from scratch.
...
The main improvment was the ability for players to play in any order (instead of waiting for others to move all their characters), and to play asynchronously, by replaying others actions at connection.
A chat was also added.

TODO replay screen video

Almost 95% of the original board game was digitalized, and the game is [alive][deephulk]{:target='_blank'} (but only in french until now).
